...
echo    (User Account Mode)
echo ========================================
echo.

:: --- User Account ---
set "SVC_USER=.\%USERNAME%"
echo Service will run as: %SVC_USER%

set "SVC_PASS="
set /p "SVC_PASS=Password for %USERNAME%: "
if "%SVC_PASS%"=="" (
    echo ERROR: Password is required for user account mode.
    goto :fail
)
...
:: --- Ensure .openclaw dir exists and grant ACL to the service user ---
if not exist "%OPENCLAW_DIR%" mkdir "%OPENCLAW_DIR%"
echo Granting %SVC_USER% full access to %OPENCLAW_DIR%...
icacls "%OPENCLAW_DIR%" /grant "%SVC_USER%:(OI)(CI)F" /T /Q >nul 2>&1

:: --- Grant "Log on as a service" right ---
echo Granting "Log on as a service" right to %SVC_USER%...
set "SEC_CFG=%TEMP%\openclaw_secpol_export.cfg"
set "SEC_DB=%TEMP%\openclaw_secpol.sdb"
secedit /export /cfg "%SEC_CFG%" /quiet >nul 2>&1

:: Find user SID via wmic
set "USER_SID="
for /f "tokens=2 delims==" %%s in ('wmic useraccount where "name='%USERNAME%'" get sid /value 2^>nul ^| findstr "SID"') do set "USER_SID=%%s"

if defined USER_SID (
    :: Check if SeServiceLogonRight already contains this SID
    findstr /c:"SeServiceLogonRight" "%SEC_CFG%" >nul 2>&1
    if !errorlevel!==0 (
        :: Line exists — append SID if not already present
        findstr /c:"%USER_SID%" "%SEC_CFG%" >nul 2>&1
        if !errorlevel! NEQ 0 (
            powershell -Command "(Get-Content '%SEC_CFG%' -Raw) -replace '(SeServiceLogonRight\s*=\s*.+)', '$1,*%USER_SID%' | Set-Content '%SEC_CFG%' -Encoding ASCII"
        )
    ) else (
        :: Line doesn't exist — add it under [Privilege Rights]
        powershell -Command "(Get-Content '%SEC_CFG%' -Raw) -replace '(\[Privilege Rights\])', \"`$1`r`nSeServiceLogonRight = *%USER_SID%\" | Set-Content '%SEC_CFG%' -Encoding ASCII"
    )
    secedit /configure /db "%SEC_DB%" /cfg "%SEC_CFG%" /quiet >nul 2>&1
    echo Granted "Log on as a service" to %SVC_USER%
) else (
    echo WARNING: Could not resolve SID for %SVC_USER%. You may need to grant "Log on as a service" manually.
)
if exist "%SEC_CFG%" del "%SEC_CFG%" >nul 2>&1
if exist "%SEC_DB%" del "%SEC_DB%" >nul 2>&1
...
echo   type %LOG_ERR%
echo.
echo NOTE: Service runs under %SVC_USER% with full user profile access.
echo       If you change your Windows password, update the service:
echo       nssm set "%SERVICE_NAME%" ObjectName "%SVC_USER%" new-password
goto :end
